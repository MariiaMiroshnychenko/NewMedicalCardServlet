...
//            "WHERE surname=? AND name=? AND patronymic=?";
    String TREATMENTS_BY_STATE = "SELECT user_data.person_id, user_data.surname_uk, user_data.surname_en, " +
            "user_data.name_uk, user_data.name_en, user_data.patronymic_uk, user_data.patronymic_en " +
            "user_data.login, user_data.`password`, user_data.`role`, user_data.photo FROM treatment " +
            "INNER JOIN user_data ON treatment.patient_id = user_data.person_id WHERE treatment.state=?";
